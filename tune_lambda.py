...
def tune_lambda(dataset_size, m1, m, reg_type, dataset_type, num_epochs,
    ...
    # Define candidate lambda values (here on a log scale)
    candidate_lambdas = [0.0, 1e-3, 1e-2, 1e-1, 1.0, 10.0]
    results_dict = {}
    best_obj = float('inf')
    best_lambda = None
    best_results = None
    ...
    for lam in candidate_lambdas:
        ...
        results = run_experiment_with_diagnostics(
            ...
            seed=seed,
            verbose=False,
            save_path=save_path  # For diagnostics plots, if any
        )
        results_dict[lam] = results
        print(f"  -> Best objective: {results['best_objective']:.4f}")
        if results['best_objective'] < best_obj:
...
def main():
    ...
    parser.add_argument("--m1", type=int, default=1, help="Value of m1")
    parser.add_argument("--m", type=int, default=3, help="Value of m")
