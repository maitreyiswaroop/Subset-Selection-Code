#!/bin/bash
#SBATCH --job-name="master_gd_pops_v10"
...
for penalty_lambda in "${penalty_lambdas[@]}"; do
  for learning_rate in "${lrs[@]}"; do
    for seed in "${seeds[@]}"; do
      SAVE_PATH="/data/user_data/mswaroop/Subset-Selection-Code/results_v10/${t2}/${seed}/"
      mkdir -p "$SAVE_PATH" "logs"
      sbatch gd_pops_v10_task.sh \
      ...
      while [ "$debug_count" -ge 3 ]; do
        echo "There are $debug_count jobs in debug partition. Waiting a minute..."
        sleep 60
        debug_count=$(squeue -u mswaroop | grep debug | wc -l)
      done
    done
