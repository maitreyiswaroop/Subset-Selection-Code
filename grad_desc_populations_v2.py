"""
grad_desc_populations_v2.py : Using LR schedulers
...
# =============================================================================
def run_experiment_multi_population_with_scheduler(pop_configs, m1, m, 
                                    dataset_size=10000,
                                    budget=None,
                                    noise_scale=0.0, num_epochs=1000, 
    ...
    elif alpha_init == "random_10":
        alpha = torch.nn.Parameter(10*torch.ones(m, device=device) + 0.1 * torch.randn(m, device=device))
    else:
        raise ValueError("alpha_init must be one of 'ones', 'random', 'random_1', or 'random_5'")
    ...
    if scheduler_type == 'step':
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=gamma, 
                                                            patience=15, threshold=1e-4, 
    ...
    for epoch in range(num_epochs):
        ...
        if verbose and (epoch % 10 == 0 or epoch == num_epochs - 1):
            print(f"Epoch {epoch}: Robust Objective = {current_obj:.6f}, LR = {lr_history[-1]:.6f}")
        ...
        if early_stopping_counter >= early_stopping_patience and epoch > 500:
            if verbose:
    ...
    if verbose:
        ...
        # Plot gradient norms
        plt.figure(figsize=(10, 6))
        gradient_norms = [np.linalg.norm(g) if g is not None else 0 for g in gradient_history]
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig(os.path.join(save_path, "learning_rate_diagnostics.png"))
        plt.close()
...
def run_experiment_multi_population(pop_configs, m1, m, 
    ...
    for epoch in range(num_epochs):
        ...
        # Early stopping logic
        if epoch > 500 and np.mean(objective_history[-3:]) >= np.mean(objective_history[-6:-3]):
            early_stopping_counter += 1
        else:
            early_stopping_counter = 0
...
def parse_args():
    ...
    parser.add_argument('--step-size', type=int, default=50,
                      help='Step size for step scheduler (epochs)')
...
def main():
    ...
    # Use the modified function with scheduler support
    results = run_experiment_multi_population_with_scheduler(
        pop_configs=pop_configs,
        m1=args.m1,
        m=args.m,
        dataset_size=args.dataset_size,
    ...
    serializable_results = {
        ...
        'lr_history': [float(lr) for lr in results.get('lr_history', [])]
    }
