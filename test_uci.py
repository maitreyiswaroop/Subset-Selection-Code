#!/usr/bin/env python3
"""
UCI Feature Importance Divergence Analysis

This script analyzes how much feature importances differ across various 
population splits in the UCI Adult dataset using XGBoost models.
...
# Import your existing UCI data loading function
from data_uci import (
...
def create_population_splits(df):
    ...
    population_splits = {
        ...
        "education": {
            "LowEdu": {"education_group": "Low"},
            "MedEdu": {"education_group": "Medium"},
            "HighEdu": {"education_group": "High"},
        },
        "work_hours": {
            "PartTime": {"hours_group": "Part-time"},
            "FullTime": {"hours_group": "Full-time"},
            "Overtime": {"hours_group": "Overtime"},
        }
    }
...
def define_outcome_variables(df):
    ...
    if 'hours_group' not in df.columns:
    ...
    # Define outcomes
    outcomes = {
        "income": {"column": "income_binary", "task": "classification"},
        "hours_per_week": {"column": "hours_per_week", "task": "regression"},
        "capital_gain": {"column": "log_capital_gain", "task": "regression"},
        "education_num": {"column": "education_num", "task": "regression"}
    }
...
def run_analysis(seed=42):
    ...
    data_dir = os.path.join(os.getcwd(), "data_uci")
    os.makedirs(data_dir, exist_ok=True)
    train_path, test_path = download_adult_dataset(data_dir)
    ...
    population_splits = create_population_splits(df)
    
    # Define outcome variables
    outcomes, df = define_outcome_variables(df)
    ...
    for i, (split_name, populations) in enumerate(population_splits.items(), 1):
        for outcome_name, outcome_info in outcomes.items():
            ...
            population_sizes = {pop_name: len(data["X"]) for pop_name, data in pop_data.items()}
            # Store results
            result = {
    ...
    results_df = pd.DataFrame([
        {
            ...
            "population_sizes": r["population_sizes"],
        }
        for r in results
    ])
...
def visualize_results(results, output_dir=None):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "uci_importance_divergence.png"), dpi=300)
    ...
    if detailed_results:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"uci_top_heatmap_{split_name}_{outcome_name}.png"), dpi=300)
...
def main():
    ...
    print("Starting UCI Feature Importance Divergence Analysis...")
    ...
    output_dir = os.path.join(current_dir, "uci_analysis_output")
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Save results to CSV
    results["ranked_df"].to_csv(os.path.join(output_dir, "uci_divergence_results.csv"), index=False)
