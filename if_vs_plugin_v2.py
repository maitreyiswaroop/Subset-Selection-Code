# if_vs_plugin_v2.py: compares IF vs plugin estimator for E[E[Y|S(alpha)]^2] 
import numpy as np
...
mu_large = full_model.predict(S_alpha)
...
for n in sample_sizes:
    ...
    mu_sub = sub_model.predict(S_sub)
    plugin_est = np.mean(mu_sub ** 2)
    plugin_estimates.append(plugin_est)
    
    # IF-based estimator on the sub-sample
    if_est = IF_estimator_squared_conditional(S_sub, Y_sub, estimator_type="rf")
...
plt.ylabel("Estimate of E[E[Y|S]^2]")
